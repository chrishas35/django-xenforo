...
class TestXFAuthBackend(TestCase):
    ...
    def test_invalid_user(self):
        ...
        with patch('xenforo.backends.connections') as mockdb:
            ...
            mockdb[settings.XENFORO_DATABASE].cursor().description = (
                ('user_id',), ('username',), ('scheme_class',), ('data',))
            self.assertEqual(
    ...
    def test_valid_user_created(self):
        ...
        with patch('xenforo.backends.connections') as mockdb:
            mockdb[settings.XENFORO_DATABASE].cursor().fetchall.return_value = (
                (123, 'TestUser', 'XenForo_Authentication_Core12', b'a:1:{s:4:"hash";s:60:"$2a$10$EnvKvs.royUb7HV2lBGAROLdwbghtS4KBlaDrkFoFa3sIAmPn6gk6";}'),)
            mockdb[settings.XENFORO_DATABASE].cursor().description = (
                ('user_id',), ('username',), ('scheme_class',), ('data',))
    ...
    def test_valid_user_existing(self):
        ...
        with patch('xenforo.backends.connections') as mockdb:
            mockdb[settings.XENFORO_DATABASE].cursor().fetchall.return_value = (
                (1234, 'TestUser', 'XenForo_Authentication_Core12', b'a:1:{s:4:"hash";s:60:"$2a$10$EnvKvs.royUb7HV2lBGAROLdwbghtS4KBlaDrkFoFa3sIAmPn6gk6";}'),)
            mockdb[settings.XENFORO_DATABASE].cursor().description = (
                ('user_id',), ('username',), ('scheme_class',), ('data',))
            user = self.backend.authenticate(username, password)
            self.assertTrue(user)
